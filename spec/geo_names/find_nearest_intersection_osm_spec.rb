...
RSpec.describe GeoNames::FindNearestIntersectionOSM do
  before :each do
  ...
  it 'raises an error on empty username' do
    ...
    GeoNames.configuration.username = nil
    expect do
      GeoNames::FindNearestIntersectionOSM.search
  ...
  it 'do not raises an error if username is in the options' do
    GeoNames.configuration.username = nil
    expect do
      GeoNames::FindNearestIntersectionOSM.search(username: username)
